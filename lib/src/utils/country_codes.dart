...
/// Get the ISO 2-letter country code from a country name or code
String getCountryCode(String input) {
  // If input is already a valid 2-letter code, return it uppercase
  if (input.length == 2) {
    return input.toUpperCase();
  }

  // Search through the country data
  final countryData = countryCodes.firstWhere(
    (country) =>
        country['name'].toString().toLowerCase() == input.toLowerCase() ||
        country['iso2_cc'].toString().toLowerCase() == input.toLowerCase(),
    orElse: () => {'iso2_cc': input.toUpperCase()},
  );

  return countryData['iso2_cc'].toString();
}
...
String? getCountryName(String code) {
  final countryData = countryCodes.firstWhere(
    (country) =>
        country['iso2_cc'].toString().toLowerCase() == code.toLowerCase(),
    orElse: () => {'name': null},
  );
...
/// Check if a country code or name is valid
bool isValidCountry(String input) {
  if (input.length == 2) {
...
/// Get the display name of a country (includes code)
String? getCountryDisplayName(String input) {
  final countryData = countryCodes.firstWhere(
    (country) =>
        country['name'].toString().toLowerCase() == input.toLowerCase() ||
        country['iso2_cc'].toString().toLowerCase() == input.toLowerCase(),
    orElse: () => {'display_name_no_e164_cc': null},
  );

  return countryData['display_name_no_e164_cc']?.toString();
}
